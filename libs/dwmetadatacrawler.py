    ...
    def crawl(self):
        ...
        if 'dir' in self.data['content'] and self.data['content']['dir']:
            for item in self.data['content']['dir']:
                ...
                _crawler = DWMetadataCrawler(str(item['id']),self.crawlroot,self.download_root,self.db, self.crawlerId)
                _crawler.open()
        if 'file' in self.data['content'] and self.data['content']['file']:
            ...
            if type(self.data['content']['file']) is dict:
                print('its a dict')
                self.db.insertRecord(self.data['content']['file']['id'],
                ...
                for item in self.data['content']['file']:
                    ...
                    try:
                        print(
                            ...
                            item['age'],
                            item['author'])
                        self.db.insertRecord(item['id'],item['title'],item['author'],item['age'])
